...
IDxcBlob* ShaderFactory::CompileShader(
	...
	if (!std::filesystem::exists(filePath)) {
		throw Lamb::Error::Code<ShaderFactory>("this file is not exists -> " + ConvertString(filePath), __func__, __FILE__, __LINE__);
	}
	...
	// 読めなかったら止める
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<ShaderFactory>("Shader Load failed", __func__, __FILE__, __LINE__);
	}
	// 読み込んだファイルの内容を設定する
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<ShaderFactory>("Danger!! Cannot use ""dxc""", __func__, __FILE__, __LINE__);
	}

	if (!shaderResult) {
		throw Lamb::Error::Code<ShaderFactory>("Create ShaderResult failed", __func__, __FILE__, __LINE__);
	}
	...
	if (shaderError != nullptr && shaderError->GetStringLength() != 0) {
		// 警告・エラーダメゼッタイ
		throw Lamb::Error::Code<ShaderFactory>(shaderError->GetStringPointer(), __func__, __FILE__, __LINE__);
	}
	...
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<ShaderFactory>("shaderResult->GetOutput() failed", __func__, __FILE__, __LINE__);
	}
	// 成功したログを出す
