...
void AnimationManager::Initialize() {
	if (instance_) {
		throw Lamb::Error::Code<AnimationManager>("Already created", __func__, __FILE__, __LINE__);
	}
...
Animations* const AnimationManager::GetAnimations(const std::string& fileName)
{
	if (not animationData_[fileName]) {
		throw Lamb::Error::Code<AnimationManager>("this file does not load -> " + fileName, __func__, __FILE__, __LINE__);
	}
