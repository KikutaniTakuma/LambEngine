...
void Pipeline::Create(
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Pipeline>("CreateGraphicsPipelineState failed", __func__, __FILE__, __LINE__);
	}
}
...
void Pipeline::Use() const {
	...
	if (!graphicsPipelineState_) {
		throw Lamb::Error::Code<Pipeline>("GraphicsPipelineState is nullptr", __func__, __FILE__, __LINE__);
	}
	...
	case D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT:
		throw Lamb::Error::Code<Pipeline>("Cannot use this primitive topology type", __func__, __FILE__, __LINE__);
		break;
	}
}
