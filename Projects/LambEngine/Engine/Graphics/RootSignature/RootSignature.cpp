...
void RootSignature::Create(D3D12_ROOT_PARAMETER* rootParamater, size_t rootParamaterSize, bool isTexture, bool isOutRangeBorder) {
	...
	if (FAILED(hr)) {
		throw Lamb::Error::Code<RootSignature>(reinterpret_cast<char*>(errorBlob->GetBufferPointer()), __func__, __FILE__, __LINE__);
	}
	// バイナリをもとに生成
	if (rootSignature_) {
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<RootSignature>("CreateRootSignature failed", __func__, __FILE__, __LINE__);
	}
}
