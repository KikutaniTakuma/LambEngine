...
Mesh MeshLoader::LoadModel(const std::string& fileName)
{
	...
	if (not scene->HasMeshes()) {
		throw Lamb::Error::Code<MeshLoader>("this file does not have meshes -> " + fileName, __func__, __FILE__, __LINE__);
	}
	...
	for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex) {
		...
		if (not mesh->HasNormals()) {
			throw Lamb::Error::Code<MeshLoader>("this file does not have normal -> " + fileName, __func__, __FILE__, __LINE__);
		}
		if (not mesh->HasTextureCoords(0)) {
			throw Lamb::Error::Code<MeshLoader>("this file does not have texcoord -> " + fileName, __func__, __FILE__, __LINE__);
		}
		...
		for (uint32_t faceIndex = 0; faceIndex < mesh->mNumFaces; ++faceIndex) {
			...
			if (face.mNumIndices != 3) {
				throw Lamb::Error::Code<MeshLoader>("this file does not support -> " + fileName, __func__, __FILE__, __LINE__);
			}
...
Animations MeshLoader::LoadAnimation(const std::string& fileName)
{
	...
	if (not (scene->mNumAnimations != 0)) {
		throw Lamb::Error::Code<MeshLoader>("this file does not have animation -> " + fileName, __func__, __FILE__, __LINE__);
	}
...
const aiScene* MeshLoader::ReadFile(Assimp::Importer& importer, const std::string& fileName)
{
	...
	if (not std::filesystem::exists(path)) {
		throw Lamb::Error::Code<MeshLoader>("this file does not find -> " + fileName, __func__, __FILE__, __LINE__);
	}
	// objかgltfではない
