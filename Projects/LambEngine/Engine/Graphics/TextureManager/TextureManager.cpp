...
void TextureManager::Initialize() {
	...
	instance_.NullPointerException<TextureManager>(__func__, __FILE__, __LINE__);
...
uint32_t TextureManager::LoadTexture(const std::string& fileName) {
	if (!std::filesystem::exists(fileName)) {
		throw Lamb::Error::Code<TextureManager>("this file is not exist -> " + fileName, __func__, __FILE__, __LINE__);
	}
