	...
	isNowThreadLoading_(false)
{
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Error::Code<TextureManager>("CreateCommandQueue() Failed", "Constructor");
	}
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Error::Code<TextureManager>("CreateCommandAllocator() Failed", "Constructor");
	}
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Error::Code<TextureManager>("CreateCommandList() Failed", "Constructor");
	}
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Error::Code<TextureManager>("CreateFence() Failed", "Constructor");
	}
	...
	if (!(fenceEvent_ != nullptr)) {
		throw Error::Code<TextureManager>("CreateEvent() Failed", "Constructor");
	}
...
Texture* const TextureManager::LoadTexture(const std::string& fileName) {
	...
	if (itr == textures_.end()) {
		...
		assert(SUCCEEDED(hr));
		if (!SUCCEEDED(hr)) {
			throw Error::Code<TextureManager>("CommandAllocator somthing error", "Reset()");
		}
		hr = commandList_->Reset(commandAllocator_.Get(), nullptr);
		assert(SUCCEEDED(hr));
		if (!SUCCEEDED(hr)) {
			throw Error::Code<TextureManager>("CommandList somthing error", "Reset()");
		}
		///
