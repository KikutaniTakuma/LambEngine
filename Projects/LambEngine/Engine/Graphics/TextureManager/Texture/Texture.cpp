...
DirectX::ScratchImage Texture::LoadTexture(const std::string& filePath) {
	if (!std::filesystem::exists(filePath)) {
		throw Lamb::Error::Code<Texture>("This file is not exist -> " + filePath, __func__, __FILE__, __LINE__);
	}
	...
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Texture>("DirectX::LoadFromWICFile() failed", __func__, __FILE__, __LINE__);
	}
	...
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Texture>("DirectX::GenerateMipMaps failed", __func__, __FILE__, __LINE__);
	}
...
ID3D12Resource* Texture::CreateTextureResource(const DirectX::TexMetadata& metaData) {
	...
	if (hr != S_OK) {
		throw Lamb::Error::Code<Texture>("somehitng error", __func__, __FILE__, __LINE__);
	}
	return resource;
}
