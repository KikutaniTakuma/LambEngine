...
void DirectXDevice::SettingAdapter() {
	...
	for (UINT i = 0;
		...
		if (hr != S_OK) {
			throw Lamb::Error::Code<DirectXDevice>("GetDesc3() Failed", "SettingAdapter()");
		}
...
ID3D12DescriptorHeap* DirectXDevice::CreateDescriptorHeap(
	...
	if (!SUCCEEDED(device_->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&descriptorHeap)))) {
		throw Lamb::Error::Code<DirectXDevice>("somthing error", "CreateDescriptorHeap()");
	}
...
ID3D12Resource* DirectXDevice::CreateBufferResuorce(size_t sizeInBytes) {
	if (!device_) {
		...
		throw Lamb::Error::Code<DirectXDevice>("device is nullptr", "CreateBufferResuorce()");
	}
	...
	if (!SUCCEEDED(hr)) {
		...
		throw Lamb::Error::Code<DirectXDevice>("somthing error", "CreateBufferResuorce()");
	}
