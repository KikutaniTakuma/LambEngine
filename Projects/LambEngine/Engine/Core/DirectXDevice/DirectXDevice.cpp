...
void DirectXDevice::SettingAdapter() {
		...
		) 
	{
		...
		if (hr != S_OK) {
			throw Lamb::Error::Code<DirectXDevice>("GetDesc3() Failed", __func__, __FILE__, __LINE__);
		}
...
void DirectXDevice::CreateDevice() {
	...
	if (device_ == nullptr) {
		throw Lamb::Error::Code<DirectXDevice>("device not found", __func__, __FILE__, __LINE__);
	}
...
ID3D12DescriptorHeap* DirectXDevice::CreateDescriptorHeap(
	...
	if (!SUCCEEDED(device_->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&descriptorHeap)))) {
		throw Lamb::Error::Code<DirectXDevice>("somthing error", __func__, __FILE__, __LINE__);
	}
...
ID3D12Resource* DirectXDevice::CreateBufferResuorce(size_t sizeInBytes) {
	if (!device_) {
		...
		throw Lamb::Error::Code<DirectXDevice>("device is nullptr", __func__, __FILE__, __LINE__);
	}
