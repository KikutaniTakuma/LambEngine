...
void Camera::Update() {
	view_.Affin(scale_, rotate_, pos_);
	...
	fov_ = std::clamp(fov_, 0.0f, 1.0f);
	projection_.PerspectiveFov(fov_, aspect, kNearClip_, farClip_);
	...
	othograohics_.Orthographic(
		-clientSize.x * 0.5f * drawScale_,
		clientSize.y * 0.5f * drawScale_,
		clientSize.x * 0.5f * drawScale_,
		-clientSize.y * 0.5f * drawScale_,
		kNearClip_, farClip_);
...
void Camera::Update(const Vector3& gazePoint) {
	...
	offset *= MakeMatrixRotate(rotate_);

	pos_ = gazePoint + offset;
