...
void Mouse::Initialize(IDirectInput8* input) {
	...
	if (!instance_) {
		throw Lamb::Error::Code<Mouse>("instance failed", __func__, __FILE__, __LINE__);
	}
}
	...
	initalizeSucceeded_(false)
{
	...
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Mouse>("CreateDevice failed", __func__, __FILE__, __LINE__);
	}

	hr = mouse_->SetDataFormat(&c_dfDIMouse2);
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Mouse>("SetDataFormat failed", __func__, __FILE__, __LINE__);
	}

	hr = mouse_->SetCooperativeLevel(WindowFactory::GetInstance()->GetHwnd(), DISCL_FOREGROUND | DISCL_NONEXCLUSIVE | DISCL_NOWINKEY);
	assert(SUCCEEDED(hr));
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Mouse>("SetCooperativeLevel failed", __func__, __FILE__, __LINE__);
	}
