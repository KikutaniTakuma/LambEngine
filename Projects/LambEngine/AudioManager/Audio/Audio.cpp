...
void Audio::Load(const std::string& fileName) {
	if (!std::filesystem::exists(std::filesystem::path{ fileName })) {
		throw Lamb::Error::Code<Audio>(("This file is not found -> " + fileName), __func__, __FILE__, __LINE__);
	}
	...
	if (extension != ".wav" and extension != ".mp3") {
		throw Lamb::Error::Code<Audio>(("This file is not supported (only ""mp3"" or ""wav"" file) -> " + fileName), __func__, __FILE__, __LINE__);
	}
	...
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Audio>("CreateSourceVoice() failed", __func__, __FILE__, __LINE__);
	}
...
void Audio::Start(float volume, bool isLoop) {
	...
	if (!pSourceVoice_) {
		...
		if (!SUCCEEDED(hr)) {
			throw Lamb::Error::Code<Audio>("SubmitSourceBuffer() failed", __func__, __FILE__, __LINE__);
		}
		hr = pSourceVoice_->SubmitSourceBuffer(&buf);
	}
	hr = pSourceVoice_->Start();
	if (!SUCCEEDED(hr)) {
		throw Lamb::Error::Code<Audio>("function is something error", __func__, __FILE__, __LINE__);
	}
