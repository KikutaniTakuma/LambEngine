...
void Player::Initialize()
{
	...
	draweManager->LoadModel("./Resources/Player/Player.obj");
	model_ = draweManager->GetModel("./Resources/Player/Player.obj");
	transform_.scale *= 5.0f;
...
void Player::Update(const Camera& camera) {
	...
	transform_.translate = offset_ * Quaternion::MakeRotateYAxis(rotate_);
	noUpDownPos_ = transform_.translate;
	transform_.rotate.y = rotate_;

	modelUpDown_ += modelUpDownSpeed_ * Lamb::DeltaTime() * (1.0f + (speed_ / (maxSpeed_ * speedScale_)) * 0.1f);
	
	transform_.translate.y += std::cos(modelUpDown_) * 0.1f;

	if (isCollisioned_) {
		color_ = Vector4ToUint({ 1.0f,1.0f,1.0f,std::abs(std::cos(chikachika_)) });
		chikachika_ += std::numbers::pi_v<float> *Lamb::DeltaTime() * 3.0f;
	}
	else {
		color_ = Vector4ToUint({ 1.0f,1.0f,1.0f,1.0f });
		chikachika_ = 0.0f;
	}
...
void Player::Attack(const Enemy& enemy) {
	...
	if (key->Pushed(DIK_SPACE) || gamepad->Pushed(Gamepad::Button::A)) {
		...
		bullets_[currentBullet_]->SetStatus(
			...
			(enemy.GetPos() - transform_.translate).Normalize(),
			bulletStateScale,
			attack_ * bulletIndex,
			0x808080ff,
			0.5f
