...
Player::Player() {
	body_ = std::make_unique<Model>();
	screw_ = std::make_unique<Model>();
	body_->LoadObj("./Resources/Player/body.obj");
	screw_->SetParent(body_.get());
	screw_->LoadObj("./Resources/Player/screw.obj");
	screw_->pos.z = -3.0f;
	kTextureSize = 70.0f;
	...
	for (int y = 0; y < height_; y++) {
		for (int x = 0; x < width_; x++) {
			...
			tileMapTexture_.at(y).at(x) = std::make_unique<Texture2D>();
			tileMapLuggageTexture_.at(y).at(x) = std::make_unique<Texture2D>();
			tileMapTexture_.at(y).at(x)->LoadTexture("./Resources/tileMap.png");
			tileMapLuggageTexture_.at(y).at(x)->LoadTexture("./Resources/luggage.png");
	...
	scheme_ = std::make_unique<Texture2D>();
	scheme_->LoadTexture("./Resources/uiImage.png");
	scheme_->scale = { 1280.0f,720.0f };

	moveAudioSound_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/move.wav", true);
	shotSound_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/shot.wav", false);
	sailSound_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/sail.wav", false);
	startSound_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/start.wav", false);

	maxPowerGauge_ = std::make_unique<Texture2D>();
	currentPowerGauge_ = std::make_unique<Texture2D>();
	...
	departure_ = std::make_unique<Texture2D>();
	departure_->LoadTexture("./Resources/InGame/departure.png");
	...
	waterBubble_->LoadSettingDirectory("water");
	particleStartSpeed_ = 0.025f;
	...
	tensPlace_ = std::make_unique<Texture2D>();
	tensPlace_->LoadTexture("./Resources/number.png");
	tensPlace_->scale = { 96.0f,96.0f };
	tensPlace_->uvSize = { 0.1f,1.0f };

	oneRank_ = std::make_unique<Texture2D>();
	oneRank_->LoadTexture("./Resources/number.png");
	...
	kmTexture_ = std::make_unique<Texture2D>();
	kmTexture_->LoadTexture("./Resources/InGame/km.png");
	kmTexture_->scale = { 128.0f,128.0f };
	kmTexture_->pos = { 548.0f,-260.0f };

	powerUI_ = std::make_unique<Texture2D>();
	powerUI_->LoadTexture("./Resources/InGame/power.png");
	powerUI_->scale = { 460.0f,60.0f };
	powerUI_->pos = { 395.0f,-320.0f };

	goalText_ = std::make_unique<Texture2D>();
	goalText_->LoadTexture("./Resources/InGame/goalText.png");
	goalText_->scale = { 520.0f,160.0f };
	goalText_->pos = { 0.0f,0.0f };

	goText_ = std::make_unique<Texture2D>();
	goText_->LoadTexture("./Resources/InGame/go.png");
	goText_->scale = { 196.0f * 1.2f,128.0f * 1.2f };
	goText_->pos = { 0.0f,0.0f };


	stageNames_ = std::make_unique<Texture2D>();
	stageNames_->LoadTexture("./Resources/InGame/stageName.png");
	...
	deathTexture_ = std::make_unique<Texture2D>();
	deathTexture_->LoadTexture("./Resources/InGame/byWhirlPool.png");
	...
	chargeAnchorPointTexture_ = std::make_unique<Texture2D>();
	chargeAnchorPointTexture_->LoadTexture("./Resources/ball.png");
...
void Player::ResourceUpdate() {
	...
	for (int y = 0; y < height_; y++) {
		for (int x = 0; x < width_; x++) {
			tileMapTexture_.at(y).at(x)->Update();
			tileMapLuggageTexture_.at(y).at(x)->Update();
		}
	}
	...
	for (auto& loopCannonNumberTexture : loopCannonNumbers_) {
		...
		loopCannonNumberTexture->Update();
	}
	scheme_->Update();
	stageNames_->Update();
	maxPowerGauge_->Update();
	currentPowerGauge_->Update();
	departure_->Update();
	tensPlace_->Update();
	oneRank_->Update();
	kmTexture_->Update();
	...
	goalText_->Update();
	goText_->Update();
	deathTexture_->Update();
	chargeAnchorPointTexture_->Update();
	// パーティクルアップデート
	waterBubble_->Update();
}
void Player::Draw(const Camera& camera) {
	body_->Draw(camera.GetViewProjection(), camera.GetPos());
	screw_->Draw(camera.GetViewProjection(), camera.GetPos());
	for (auto& barrel : barrels_) {
		barrel->Draw(camera.GetViewProjection(), camera.GetPos());
	}
	for (auto& bone : bones_) {
		bone->Draw(camera.GetViewProjection(), camera.GetPos());
	}
	for (size_t i = 0; auto & sail : sails_) {
		if (isCustomize_) {
			sail->Draw(camera.GetViewProjection(), camera.GetPos());
		}
	...
	for (auto& cannon : cannons_) {
		cannon->Draw(camera.GetViewProjection(), camera.GetPos());
	}
	for (auto& loopCannon : loopCannons_) {
		loopCannon->Draw(camera.GetViewProjection(), camera.GetPos());
	}
#ifdef _DEBUG
	...
	if (isStart_) {
		waterBubble_->Draw(camera.rotate, camera.GetViewProjection());
	}
}
...
void Player::DrawUI(const Camera& camera) {
	...
	if (!isCustomize_ && !isGoal_) {
		powerUI_->Draw(camera.GetOthographics());
		currentPowerGauge_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		//maxPowerGauge_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
	}
	if (isCharge_ && !isStart_ && !isGoal_ && !isCustomize_) {
		departure_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
//#ifdef _DEBUG
		chargeAnchorPointTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
//#endif // _DEBUG

	}
	else if (!isStart_ && !isGoal_ && !isCustomize_) {
		goalText_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
	}
	else if (isStart_ && !isGoal_ && !isCustomize_) {
		scheme_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		stageNames_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		if (tensPlace_->uvPibot.x != 0.0f) {
			tensPlace_->Draw(camera.GetOthographics());
		}
		oneRank_->Draw(camera.GetOthographics());
		kmTexture_->Draw(camera.GetOthographics());
		goText_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		deathTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
	}
	// チャージ画面で映したくないもの
		...
		for (int y = 0; y < height_; y++) {
			for (int x = 0; x < width_; x++) {
				if (tileMap_.at(y).at(x).type == Type::kNotPut) {
					tileMapLuggageTexture_.at(y).at(x)->Draw(camera.GetOthographics());
				}
				else {
					tileMapTexture_.at(y).at(x)->Draw(camera.GetOthographics());
				}
			}
		}
		for (auto& sailTexture : sailsTextures_) {
			sailTexture->Draw(camera.GetOthographics());
		}
		for (auto& cannonTexture : cannonsTextures_) {
			cannonTexture->Draw(camera.GetOthographics());
		}
		for (auto& loopCannonTexture : loopCannonsTextures_) {
			loopCannonTexture->Draw(camera.GetOthographics());
		}
		if (isCustomize_) {
			for (auto& sailIntervalTexture : sailsIntervalTextures_) {
				sailIntervalTexture->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}
			for (auto& cannonIntervalTexture : cannonsIntervalTextures_) {
				cannonIntervalTexture->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}
			for (auto& loopCannonIntervalTexture : loopCannonsIntervalTextures_) {
				loopCannonIntervalTexture->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}

			for (auto& sailNumberTexture : sailNumbers_) {
				sailNumberTexture->Draw(camera.GetOthographics());
			}
			for (auto& cannonNumberTexture : cannonNumbers_) {
				cannonNumberTexture->Draw(camera.GetOthographics());
			}
			for (auto& loopCannonNumberTexture : loopCannonNumbers_) {
				loopCannonNumberTexture->Draw(camera.GetOthographics());
			}
		}
		else {
...
void Player::SetCannonIntervalNumbers(int32_t index) {
	...
	cannonNumbers_.at(index)->Update();
}
...
void Player::SetLoopCannonIntervalNumbers(int32_t index) {
	...
	loopCannonNumbers_.at(index)->Update();
}
...
void Player::UpdateRotate(const Vector3& vector) {
	...
	// 進行方向の向きによってパーティクルを出す向きを返る
	waterBubble_->spdRotate = vector_;
	waterBubble_->emitteRotate = vector_;
}
	...
	// 音
	{
		if (velocity_ > 0.0f) {
			if (!moveAudioSound_->IsStart()) {
				moveAudioSound_->Start(seVolume_);
			}
		}
		else {
			velocity_ = 0.0f;
...
#pragma endregion
				...
				Vector3 cannonPos = (cannon->GetWorldMatrix())[3].GetVector3();
				BulletManager::GetInstance()->Create(cannonPos, -cannonVector);
				shotSound_->Start(seVolume_);
		...
		if (itemName == "objectData") {
			...
			for (nlohmann::json::iterator itObjectData = itObject->begin(); itObjectData != itObject->end(); ++itObjectData) {
				...
				if (objectName.find("Sail") != std::string::npos) {
					...
					sailNumber->LoadTexture("./Resources/outLineNumber.png");
					...
					if (cannons_.back()->rotate.y == 0 * std::numbers::pi_v<float> / 180.0f) {
						...
						cannonNumber->LoadTexture("./Resources/outLineNumber.png");
