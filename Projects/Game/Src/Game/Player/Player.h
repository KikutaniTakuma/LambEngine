...
public:
	...
	Texture2D* GetLoopCannonsTextures(int32_t index) { return loopCannonsTextures_.at(index).get(); }

	void AddBarrel(Model* model) { barrels_.emplace_back(std::move(model)); }
	void AddSails(Model* model) { sails_.emplace_back(std::move(model)); }
	void AddCannons(Model* model) { cannons_.emplace_back(std::move(model)); }
	void AddLoopCannons(Model* model) { loopCannons_.emplace_back(std::move(model)); }
	void AddBone(Model* model) { bones_.emplace_back(std::move(model)); }
	void AddSailTextures(Texture2D* texture) { sailsTextures_.emplace_back(std::move(texture)); }
	void AddCannonsTextures(Texture2D* texture) { cannonsTextures_.emplace_back(std::move(texture)); }
	void AddLoopCannonsTextures(Texture2D* texture) { loopCannonsTextures_.emplace_back(std::move(texture)); }
...
#pragma endregion
	...
	void AddSailsIntervalTextures(Texture2D* texture) { sailsIntervalTextures_.emplace_back(std::move(texture)); }
	void AddSailNumbers(Texture2D* texture) { sailNumbers_.emplace_back(std::move(texture)); }

	void AddCannonIntervalTextures(Texture2D* texture) { cannonsIntervalTextures_.emplace_back(std::move(texture)); }
	void AddCannonsNumbers(Texture2D* texture) { cannonNumbers_.emplace_back(std::move(texture)); }

	void AddLoopCannonIntervalTextures(Texture2D* texture) { loopCannonsIntervalTextures_.emplace_back(std::move(texture)); }
	void AddLoopCannonsNumbers(Texture2D* texture) { loopCannonNumbers_.emplace_back(std::move(texture)); }
...
#pragma region キクタニが追加しました
	// 水面の泡のパーティクル
	std::unique_ptr<Particle> waterBubble_;
	// パーティクルを出す速度基準
	float particleStartSpeed_;
	// エミッタのオフセット
