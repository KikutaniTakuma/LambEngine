...
void Customize::Initialize() {
	...
	cursorBone_ = std::make_unique<Model>();
	cursorSail_ = std::make_unique<Model>();
	cursorCannon_ = std::make_unique<Model>();
	cursorLoopCannon_ = std::make_unique<Model>();

	cursorBone_->SetParent(player_->GetBody());
	cursorBone_->LoadObj("./Resources/Player/bone.obj");
	cursorBone_->SetParent(player_->GetBody());
	cursorSail_->LoadObj("./Resources/Player/sail.obj");
	cursorSail_->SetParent(player_->GetBody());
	cursorCannon_->LoadObj("./Resources/Player/cannon.obj");
	cursorCannon_->SetParent(player_->GetBody());
	cursorLoopCannon_->LoadObj("./Resources/Player/loopCannon.obj");
	cursorLoopCannon_->SetParent(player_->GetBody());

	customizeBackGroundTexture_ = std::make_unique<Texture2D>();
	customizeBackGroundTexture_->LoadTexture("./Resources/InGame/customizeBackGround.png");
	customizeBackGroundTexture_->scale = { 1280.0f,720.0f };

	customizeBaseTexture_ = std::make_unique<Texture2D>();
	cursorTexture_ = std::make_unique<Texture2D>();
	cursorSailTexture_ = std::make_unique<Texture2D>();
	cursorCannonTexture_ = std::make_unique<Texture2D>();
	cursorLoopCannonTexture_ = std::make_unique<Texture2D>();

	customizeBaseTexture_->LoadTexture("./Resources/InGame/customizeBase.png");
	cursorTexture_->LoadTexture("./Resources/InGame/cursor.png");
	cursorSailTexture_->LoadTexture("./Resources/InGame/customizeSail.png");
	cursorCannonTexture_->LoadTexture("./Resources/InGame/customizeCannon.png");
	cursorLoopCannonTexture_->LoadTexture("./Resources/InGame/customizeLoopCannon.png");
	...
	cursorNumbers_ = std::make_unique<Texture2D>();
	cursorNumbers_->LoadTexture("./Resources/outLineNumber.png");
	...
	explanationCursorTexture_ = std::make_unique<Texture2D>();
	explanationCannonTexture_ = std::make_unique<Texture2D>();
	explanationLoopCannonTexture_ = std::make_unique<Texture2D>();
	explanationSailTexture_ = std::make_unique<Texture2D>();

	explanationCursorTexture_->LoadTexture("./Resources/InGame/explanationCursor.png");
	explanationCannonTexture_->LoadTexture("./Resources/InGame/explanationCannon.png");
	explanationLoopCannonTexture_->LoadTexture("./Resources/InGame/explanationLoopCannon.png");
	explanationSailTexture_->LoadTexture("./Resources/InGame/explanationSail.png");
	...
	startTexture_ = std::make_unique<Texture2D>();
	startTexture_->LoadTexture("./Resources/InGame/start.png");
	startTexture_->scale = { 220.0f,100.0f };
	startTexture_->pos = { 530.0,-320.0f,0.0f };

	gearTexture_ = std::make_unique<Texture2D>();
	gearTexture_->LoadTexture("./Resources/InGame/gear.png");
	gearTexture_->scale = { 280.0f,280.0f };
	gearTexture_->pos = { 0.0,365.0f,0.0f };

	maxSailCountTexture_ = std::make_unique<Texture2D>();
	maxCannonCountTexture_ = std::make_unique<Texture2D>();
	maxLoopCannonCountTexture_ = std::make_unique<Texture2D>();
	maxSailCountTexture_->LoadTexture("./Resources/outLineNumber.png");
	maxCannonCountTexture_->LoadTexture("./Resources/outLineNumber.png");
	maxLoopCannonCountTexture_->LoadTexture("./Resources/outLineNumber.png");

	toStageSelectTexture_ = std::make_unique<Texture2D>();
	toStageSelectTexture_->LoadTexture("./Resources/OutGame/toStageSelect.png");
	toStageSelectTexture_->scale = { 220.0f,100.0f };
	toStageSelectTexture_->pos = { -530.0,-312.0f,0.0f };

	auto csv = CsvLoad("./Datas/CustomizeMapData/" + StageManager::GetCurrentStageString() + "/CustomizeMapData.csv");
	for (int y = 0; y < height_; y++) {
		for (int x = 0; x < width_; x++) {
			if (csv.at(y).at(x) == 1) {
				Model* model = new Model();
				model->LoadObj("./Resources/Player/barrel.obj");
	...
	pushA_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/set.wav", false);
	pushB_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/delete.wav", false);
	pushY_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/count.wav", false);
	pushLBRB_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/rotate.wav", false);
	pushLTRT_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/changeItem.wav", false);
	pushStart_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/ready.wav", false);
	moveTile_ = AudioManager::GetInstance()->LoadWav("./Resources/Sound/moveTile.wav", false);
...
void Customize::Update() {
	...
	customizeBackGroundTexture_->Update();
	gearTexture_->Update();
	customizeBaseTexture_->Update();

	cursorTexture_->Update();

	cursorNumbers_->Update();
	cursorSailTexture_->Update();
	cursorCannonTexture_->Update();
	cursorLoopCannonTexture_->Update();

	explanationCursorTexture_->Update();
	explanationCannonTexture_->Update();
	explanationLoopCannonTexture_->Update();
	explanationSailTexture_->Update();

	maxSailCountTexture_->Update();
	maxCannonCountTexture_->Update();
	maxLoopCannonCountTexture_->Update();

	startTexture_->Update();
	toStageSelectTexture_->Update();
	...
	for (auto& loopCannon : loopCannonInterval_) {
		...
		loopCannon.left->Update();
		loopCannon.right->Update();
	}
#ifdef _DEBUG
	Debug();
#endif // _DEBUG
...
void Customize::Draw(const Camera& camera) {
			...
			if (angle_ == 90.0f || angle_ == 270.0f) {
				if (cursor_y - 1.0f >= 0.0f && cursor_y + 1.0f < height_) {
					if (player_->GetMapData(cursor_x, cursor_y).type == Player::kNone &&
						...
						player_->GetMapData(cursor_x, cursor_y + 1).type == Player::kNone) {
						cursorSail_->Draw(camera.GetViewProjection(), camera.GetPos());
						cursorBone_->Draw(camera.GetViewProjection(), camera.GetPos());
					}
				}
			}
			else {
		...
		case Customize::kCannon:
			if (angle_ == 0.0f) {
				if (cursor_y - 1.0f >= 0.0f) {
					if (player_->GetMapData(cursor_x, cursor_y).type == Player::kNone &&
						player_->GetMapData(cursor_x, cursor_y - 1).type == Player::kNone) {
						cursorCannon_->Draw(camera.GetViewProjection(), camera.GetPos());
					}
				}
			}
		...
		case Customize::kLoopCannon:
			if (angle_ == 0.0f) {
				if (cursor_y - 1.0f >= 0.0f) {
					if (player_->GetMapData(cursor_x, cursor_y).type == Player::kNone &&
						player_->GetMapData(cursor_x, cursor_y - 1).type == Player::kNone) {
						cursorLoopCannon_->Draw(camera.GetViewProjection(), camera.GetPos());
	...
	gearTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false);
	customizeBaseTexture_->Draw(camera.GetOthographics());
...
void Customize::PostDraw(const Camera& camera) {
	for (int index = 0; auto & sail : sailInterval_) {
		if (player_->GetSailTextures(index)->Collision(ChangeCursorToTexturePos())) {
			sail.left->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			sail.right->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		}
		index++;
	}
	for (int index = 0; auto & cannon : cannonInterval_) {
		if (player_->GetCannonsTextures(index)->Collision(ChangeCursorToTexturePos())) {
			cannon.left->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			cannon.right->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		}
		index++;
	}
	for (int index = 0; auto & loopCannon : loopCannonInterval_) {
		if (player_->GetLoopCannonsTextures(index)->Collision(ChangeCursorToTexturePos())) {
			loopCannon.left->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			loopCannon.right->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
		}
		index++;
	}
	if (player_->GetIsCustomize()) {
		...
		case Customize::kNone:
			break;
		case Customize::kSail:
			explanationSailTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			if ((ChangeCursorToTileMap().x != -1.0f ||
				ChangeCursorToTileMap().y != -1.0f)) {
				cursorSailTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
				//cursorNumbers_->pos.z = 0.0f;
				//cursorNumbers_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}
			break;
		case Customize::kCannon:
			explanationCannonTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);

			if ((ChangeCursorToTileMap().x != -1.0f ||
				ChangeCursorToTileMap().y != -1.0f)) {
				cursorCannonTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
				//cursorNumbers_->pos.z = 0.0f;
				//cursorNumbers_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}
			break;
		case Customize::kLoopCannon:
			explanationLoopCannonTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);

			if ((ChangeCursorToTileMap().x != -1.0f ||
				ChangeCursorToTileMap().y != -1.0f)) {
				cursorLoopCannonTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
				//cursorNumbers_->pos.z = 0.0f;
				//cursorNumbers_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			}
			break;
		case Customize::kCursor:
			explanationCursorTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
			break;
		case Customize::kTypeCount:
			break;
		default:
	...
	//cursorTexture_->Draw(camera.GetOthographics());

	startTexture_->Draw(camera.GetOthographics());
	toStageSelectTexture_->Draw(camera.GetOthographics());
	maxSailCountTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
	maxCannonCountTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
	maxLoopCannonCountTexture_->Draw(camera.GetOthographics(), Pipeline::Normal, false, false);
}
	...
	// ゲームパット
	{
		if (isInput_ &&
			...
			if (std::fabs(gamepad->GetStick(Gamepad::Stick::LEFT_X)) > std::fabs(gamepad->GetStick(Gamepad::Stick::LEFT_Y))) {
				...
				if (gamepad->GetStick(Gamepad::Stick::LEFT_Y) > 0.0f) {
					pos.y = 1.0f;
				}
				else {
					pos.y = -1.0f;
		...
		if (gamepad->GetButton(Gamepad::Button::UP) &&
			!gamepad->GetPreButton(Gamepad::Button::UP)) {
			moveTile_->Start(seVolume_);
			cursor_.y += 1.0f;
		}
		if (gamepad->GetButton(Gamepad::Button::DOWN) &&
			!gamepad->GetPreButton(Gamepad::Button::DOWN)) {
			moveTile_->Start(seVolume_);
			cursor_.y -= 1.0f;
		}
		if (gamepad->GetButton(Gamepad::Button::RIGHT) &&
			!gamepad->GetPreButton(Gamepad::Button::RIGHT)) {
			moveTile_->Start(seVolume_);
			cursor_.x += 1.0f;
		}
		if (gamepad->GetButton(Gamepad::Button::LEFT) &&
			!gamepad->GetPreButton(Gamepad::Button::LEFT)) {
			moveTile_->Start(seVolume_);
			cursor_.x -= 1.0f;
		}

		cursor_.x = std::clamp(cursor_.x, 0.0f, width_ - 1.0f);
		cursor_.y = std::clamp(cursor_.y, 0.0f, height_ - 1.0f);
		// カウントリセット
		{
			if (!isInput_) {
				inputCount_++;
		...
		if (isTriggerInput_ &&
			gamepad->GetTriger(Gamepad::Triger::LEFT)) {
			pushLTRT_->Start(seVolume_);
			isTriggerInput_ = false;
			triggerInputCount_ = 0;
			switch (currentType_) {
			case Customize::kNone:
				break;
		...
		// カウントリセット
		{
			if (!isTriggerInput_) {
				triggerInputCount_++;
				if (triggerInputCount_ >= maxInput_) {
					isTriggerInput_ = true;
					triggerInputCount_ = 0;
				}
			}
			if (gamepad->GetTriger(Gamepad::Triger::LEFT) == 0.0f &&
				gamepad->GetTriger(Gamepad::Triger::RIGHT) == 0.0f) {
				isTriggerInput_ = true;
				triggerInputCount_ = 0;
			}
		}
...
void Customize::SetSail() {
	...
	Model* sail = new Model();
	sail->LoadObj("./Resources/Player/sail.obj");
	...
	Model* bone = new Model();
	bone->LoadObj("./Resources/Player/bone.obj");
	...
	Texture2D* sailTexture = new Texture2D();
	sailTexture->LoadTexture("./Resources/InGame/customizeSail.png");
	...
	Texture2D* sailIntervalTexture = new Texture2D();
	sailIntervalTexture->LoadTexture("./Resources/white2x2.png");
	...
	Texture2D* sailNumber = new Texture2D();
	sailNumber->LoadTexture("./Resources/outLineNumberS.png");
	...
	std::unique_ptr<Texture2D> left = std::make_unique<Texture2D>();
	std::unique_ptr<Texture2D> right = std::make_unique<Texture2D>();

	left->LoadTexture("./Resources/InGame/minus.png");
	right->LoadTexture("./Resources/InGame/plus.png");
...
void Customize::SetCannon(float x, float y) {
	...
	Model* cannon = new Model();
	cannon->LoadObj("./Resources/Player/cannon.obj");
	...
	cannonTexture->LoadTexture("./Resources/InGame/customizeCannon.png");
	...
	Texture2D* cannonsIntervalTexture = new Texture2D();
	cannonsIntervalTexture->LoadTexture("./Resources/white2x2.png");
	...
	Texture2D* cannonNumber = new Texture2D();
	cannonNumber->LoadTexture("./Resources/outLineNumberS.png");
...
void Customize::SetLoopCannon(float x, float y) {
	...
	player_->SetLoopCannonCount(1);
	Model* cannon = new Model();
	cannon->LoadObj("./Resources/Player/loopCannon.obj");
	cannon->SetParent(player_->GetBody());
	cannon->rotate.y = angle_ * std::numbers::pi_v<float> / 180.0f;
	cannon->scale.x = setObjectScale_;
	cannon->scale.y = setObjectScale_;
	...
	Texture2D* cannonTexture = new Texture2D();
	cannonTexture->LoadTexture("./Resources/InGame/customizeLoopCannon.png");
	cannonTexture->pos = { (ChangeCursorToTileMap().x - int(float(width_) * 0.5f) + x) * player_->GetTileMapSize() + player_->GetTileMapOffset().x , (ChangeCursorToTileMap().y - int(float(height_) * 0.5f) - y) * player_->GetTileMapSize() + player_->GetTileMapOffset().y };
	cannonTexture->scale = { player_->GetTileMapSize() * decisionSize_ ,player_->GetTileMapSize() * 2.0f * decisionSize_ };
	cannonTexture->rotate.z = angle_ * std::numbers::pi_v<float> / 180.0f;
	cannonTexture->color = Vector4ToUint(setColor_);
