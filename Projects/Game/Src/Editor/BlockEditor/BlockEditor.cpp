...
void BlockEditor::Update() {
	if (ImGui::TreeNode("BlockEditor")) {
		...
		for (size_t i = 0; auto & block : blockManager_->GetBlocks()) {
			...
			if (ImGui::TreeNode(("Block:" + std::to_string(i)).c_str())) {
				...
				ImGui::DragFloat3(("scale:" + std::to_string(i)).c_str(), &blockScale.x, 1.0f);
		...
		if (ImGui::TreeNode("CreateBlock")) {
			...
			if (ImGui::Button("Create")) {
				blockManager_->Create(scale_, rotate_, position_);
			}
			ImGui::TreePop();
			model_->scale = scale_;
...
void BlockEditor::LoadFile(const std::string& stageName) {
...
void BlockEditor::SaveFile(const std::string& stageName) {
	nlohmann::json root;
